...
class FrontDoorTests(unittest.TestCase):
    ...
    def test_generic_bug_words_do_not_route_to_deploy_plugin(self) -> None:
        # Hermetic copy of the real regression: the installed vercel plugin's
        # capability text contains "login" and "crash", which hijacked the
        # generic request "Fix the login crash" on a live machine.
        plugins = [
            {
                "name": "vercel",
                ...
                "skills": ["vercel"],
                "capability_text": (
                    "deploy vercel preview production logs monitoring "
                    "crash login runtime errors"
                ),
                "manifest": "plugin.json",
            }
        ]

        result = front_door.choose_plugin_route("Fix the login crash", plugins)

        self.assertFalse(result["matched"])
        self.assertEqual(result["source"], "acc")

    def test_explicit_plugin_name_still_routes(self) -> None:
        plugins = [
            {
                "name": "vercel",
                "description": "Deploy and monitor apps on Vercel.",
                "skills": ["vercel"],
                "capability_text": "deploy vercel preview production logs",
                "manifest": "plugin.json",
            }
        ]

        result = front_door.choose_plugin_route("deploy this on vercel", plugins)
    ...
    def test_resolve_entry_mode_skips_disk_io_for_nonexistent_project_root(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            fake_root = Path(tmp) / "does-not-exist"

            result = front_door.resolve_entry_mode(
                "I want to build a website", fake_root
            )

            self.assertEqual(result["entry_mode_source"], "classified")
            self.assertFalse(fake_root.exists())

    def test_route_request_skips_disk_io_for_nonexistent_project_root(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            fake_root = str(Path(tmp) / "does-not-exist")

            result = front_door.route_request(
                "I want to build a website", {"project_root": fake_root}
            )

            self.assertEqual(result["entry_mode_source"], "classified")
            self.assertFalse(Path(fake_root).exists())
