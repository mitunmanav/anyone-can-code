...
STATE_WRITE_BACKOFF_SECONDS = 0.15
...
def prompt_log_path(repo_root: Path) -> Path:
    return ensure_project_layout(repo_root)["state"] / "prompt-log.jsonl"
...
def write_state(repo_root: Path, updates: dict) -> dict:
    ...
    # Transient file locks (antivirus, thread creation) can fail one atomic
    # write; the transaction rolls back cleanly, so a bounded retry is safe.
    for attempt in range(1, STATE_WRITE_ATTEMPTS + 1):
        try:
            return canonical_state.update_canonical_state(repo_root, normalized)
        except canonical_state.StateTransactionError:
            if attempt == STATE_WRITE_ATTEMPTS:
                raise
            time.sleep(STATE_WRITE_BACKOFF_SECONDS * attempt)
...
# Minor-9: JSONL ledgers (signal log, receipts, etc.) share one append
# helper and are otherwise unbounded. Rotate on append once a file exceeds
# this many lines, keeping only the newest ones.
JSONL_ROTATE_THRESHOLD = 500
JSONL_ROTATE_KEEP = 200
...
def read_recent_jsonl(path: Path, limit: int = 25, signal_type: str | None = None) -> list[dict]:
    """Read the last `limit` jsonl rows.

    Minor-8: when `signal_type` is given, filter by type FIRST over the
    whole file, then take the last `limit` matches -- so other signal
    types (e.g. tool_used) can never crowd the type you asked for out of
    the window. Callers that don't pass signal_type keep the old
    last-N-raw-lines behavior.
    """
    if not path.exists():
        return []
    lines = path.read_text(encoding="utf-8").splitlines()
...
def _normalize_command(command: str) -> str:
    return " ".join(str(command).split()).lower()


def record_command_result(project_root: Path, command: str, exit_code: int) -> None:
    """Log one command result (B11 repeat-failure guard)."""
    append_jsonl(
        signal_log_path(Path(project_root)),
        {
            "timestamp": utc_now(),
            "signal_type": "command_result",
...
def repeat_failure(project_root: Path, command: str, window: int = 20) -> bool:
    """True when SAME normalized command failed >=2 times in last `window`
    command_result signals (Minor-8: filtered by type BEFORE windowing, so
    tool_used rows never evict command_result rows from the window)."""
    recent = read_recent_jsonl(
        signal_log_path(Path(project_root)), limit=window, signal_type="command_result"
    )
    needle = _normalize_command(command)
    failures = [
        row
        for row in recent
        if row.get("command") == needle and row.get("exit_code", 0) != 0
    ]
