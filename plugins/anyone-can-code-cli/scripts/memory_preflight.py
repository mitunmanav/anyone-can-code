...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="ACC memory preflight",
        epilog="Example: memory_preflight.py \"fix login bug\" --project-root .",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument("request", help="User request to recall memory for")
    parser.add_argument("--project-root", default=os.getcwd(), help="Project root directory (default: current working directory)")
    parser.add_argument("--limit", type=int, default=DEFAULT_LIMIT)
    args = parser.parse_args()

    if not args.request.strip():
        print("memory_preflight: request text required. Example: memory_preflight.py \"fix login bug\" --project-root .")
        sys.exit(2)
