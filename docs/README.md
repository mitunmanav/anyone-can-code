# Docs map

Current product version: see [`plugins/anyone-can-code/.codex-plugin/plugin.json`](../plugins/anyone-can-code/.codex-plugin/plugin.json)  
Install and everyday use: [root README](../README.md)

## Public (this repo)

| Doc | What |
|-----|------|
| [PRIVACY.md](PRIVACY.md) | What the plugin stores; no telemetry |
| [TERMS.md](TERMS.md) | License, warranty, responsibilities |
| [CREDITS.md](CREDITS.md) | Attribution and links |
| [ACC-OPERATING-CONTRACT.md](ACC-OPERATING-CONTRACT.md) | Operating rules for shipped product |
| [plugin README](../plugins/anyone-can-code/README.md) | Skills, memory, tests |
| [MEMORY-CONTRACT](../plugins/anyone-can-code/MEMORY-CONTRACT.md) | Memory backend rules |
| [VALIDATION](../plugins/anyone-can-code/VALIDATION.md) | Acceptance checklist |
| [IMPLEMENTATION-SOURCE-OF-TRUTH](../plugins/anyone-can-code/IMPLEMENTATION-SOURCE-OF-TRUTH.md) | What shipped |
| [automations](../plugins/anyone-can-code/automations/README.md) | Optional scheduled prompts |
| [CHANGELOG](../CHANGELOG.md) | Release history |

## Community (GitHub)

| Doc | What |
|-----|------|
| [CONTRIBUTING](../.github/CONTRIBUTING.md) | How to help; humans vs AI agents |
| [SUPPORT](../.github/SUPPORT.md) | Where to get help |
| [DISCUSSIONS](../.github/DISCUSSIONS.md) | Q&A / Ideas / Show and tell map |
| [ISSUES](../.github/ISSUES.md) | Bug / install / feature forms map |
| [SECURITY](../.github/SECURITY.md) | Private vulnerability reports |
| [MAINTAINER](../.github/MAINTAINER.md) | Maintainer cheat sheet (Mitun) |
| [CODE_OF_CONDUCT](../.github/CODE_OF_CONDUCT.md) | Community norms |

## Contributor / agent (repo root)

| Doc | What |
|-----|------|
| [AGENTS.md](../AGENTS.md) | Agent context for this repo |
| [DEVELOPMENT-WORKFLOW.md](../DEVELOPMENT-WORKFLOW.md) | Maintainer delivery flow |

## Local-only (not published)

Some maintainer notes live only on the maintainer machine (gitignored): improvement loops, trial notes, plans, plugin goals/rulebook drafts, session handoffs. If those conflict with the public files above, **trust the public files**.
