...
- `06 Task Map`: human-readable mirror of tasks and dependencies.
...
Obsidian owns product history, decisions, and task state. It explains history and
decisions and owns technical task state.
...
6. Update `06 Task Map` when technical requirements or tasks changed.
7. Treat Obsidian as continuity memory when chat/thread context is lost.
8. Mirror task status in `06 Task Map` and `02 Current Status`.
9. Reconcile product intent into Obsidian before more implementation.
...
- `development-system`: repo workflow docs, Obsidian, worktree process,
...
user explicitly expands scope. Use
Obsidian as the technical control plane and history/evidence brain.
...
| Obsidian project brain | Related Obsidian notes | note count/link check; status consistency scan |
| Plugin source or hook behavior | Tests, Doctor, runtime cache check, Obsidian evidence, repo docs if workflow/setup changed | tests pass; Doctor pass; cache source/hash or behavior proof |
| Runtime cache | Obsidian evidence; source remains authoritative | source/cache hash or behavior proof; no source drift |
| Worktree sync/merge | Obsidian status/evidence/git ledger | root/dev/test status and hashes; candidate validation |
...
with pass/fail. It does not change files, branches, Obsidian,
runtime cache, GitHub, PRs, tags, releases, or plugin behavior.

### Evidence receipt

Every completed reliability or plugin task should leave this proof shape in
Obsidian:
...
- Never create duplicate Obsidian notes for the same product request.
...
- Use inspiration repos only as reference material. Put cloned references under `C:\Users\<user>\Desktop\Plugin development\inspiration`.
